 ...
 */
const OPML_PATH = "https://andre-vincent.github.io/rss-flux/feeds.opml";
...
async function init() {
  try {
    ...
    outlines.forEach((outline) => {
      ...
      btn.onclick = () =>
        loadRSS(outline.getAttribute("xmlUrl"), btn.textContent);
    ...
    feedMenu.innerHTML =
      '<p class="error">Erreur de chargement du fichier OPML .</p>';
  }
}
...
async function loadRSS(url, title) {
  ...
  try {
    ...
    if (data.status === "ok") {
      ...
      articlesContainer.innerHTML = data.items
        .map((item) => {
    ...
    articlesContainer.innerHTML =
      '<p class="error">Impossible de lire ce flux RSS .</p>';
  }
}
